...
static bool stepcounter_can_read(struct calvinsys_obj_t *obj)
{
	android_sensor_data_t *data_acc = (android_sensor_data_t *)obj->state;
	return data_acc->data != NULL && data_acc->data_size > 0;
}

static result_t stepcounter_read(struct calvinsys_obj_t *obj, char **data, size_t *size)
{
	android_sensor_data_t *data_acc = (android_sensor_data_t *)obj->state;
...
static result_t stepcounter_close(calvinsys_obj_t *obj)
{
	android_sensor_data_t *data_acc = (android_sensor_data_t *)obj->state;
...
static calvinsys_obj_t *stepcounter_open(calvinsys_handler_t *handler, char *data, size_t len, void *state, uint32_t id, const char* capability_name)
{
...
result_t calvinsys_stepcounter_create(calvinsys_t **calvinsys, const char *name)
{
