...
static result_t send_upstream_platform_message(const transport_client_t *transport_client, char *cmd, char *data, size_t data_size)
{
  int res = 0;
  transport_fcm_client_t *fcm_client = (transport_fcm_client_t *)transport_client->client_state;

	memcpy(data + 4, cmd, 2); // set 2 byte command type

	res = write(((android_platform_t *)fcm_client->node->platform)->upstream_platform_fd[1], data, data_size);
	if (res <= 0)
		log_error("Could not write upstream message");
	else
		log_debug("Wrote upstream message of size: %d",  data_size);

	return res > 0;
}

static result_t send_downstream_platform_message(const node_t *node, char *cmd, char *data, size_t data_size)
{
	int res = 0;
	char buffer[TRANSPORT_RX_BUFFER_SIZE];

	transport_set_length_prefix(buffer, data_size + 2);

	memset(buffer, 0, TRANSPORT_RX_BUFFER_SIZE);
	memcpy(buffer + 4, cmd, 2);
	memcpy(buffer + 6, data, data_size);
	res = write(((android_platform_t*)node->platform)->downstream_platform_fd[1], buffer, data_size+6);
	if (res <= 0)
		log_error("Could not write downstream message");
	else
		log_debug("Wrote downstream message");
...
{
    return node_handle_message(node, payload_data, size);
}
...
{
	return transport_join(node, node->transport_client);
}
...
static result_t external_calvinsys_init(calvinsys_t* calvinsys)
{
	...
	transport_set_length_prefix(buffer, len);

	return send_upstream_platform_message(calvinsys->node->transport_client, INIT_EXTERNAL_CALVINSYS, buffer, len);
}
...
static result_t external_calvinsys_release(calvinsys_t* calvinsys)
{
	...
	log("Send upstream destroy message for: %s", calvinsys->name);
	strcpy(buffer + 6, calvinsys->name);
	transport_set_length_prefix(buffer, len);

	return send_upstream_platform_message(calvinsys->node->transport_client, DESTROY_EXTERNAL_CALVINSYS, buffer, len);
}
...
static result_t external_calvinsys_input(calvinsys_t* calvinsys, char* command, char* data, size_t data_size)
{
	...
	transport_set_length_prefix(buffer, buf - buffer);

	return send_upstream_platform_message(calvinsys->node->transport_client, DESTROY_EXTERNAL_CALVINSYS, buffer, buf - buffer);
}
...
static result_t platform_android_handle_fcm_data(node_t* node, char *buffer, size_t size)
{
	...
	msg_size = transport_get_message_len(buffer);
	log("Got downstream msg '%s' message size '%zu'", buffer, msg_size);
	...
	for (i = 0; i < NBR_OF_COMMANDS; i++) {
		if (strcmp(platform_command_handlers[i].command, cmd) == 0) {
			...
			return platform_command_handlers[i].handler(node, buffer + 6, msg_size - 2);
		}
	}
