...
static result_t actor_pressure_init(actor_t **actor, list_t *attributes)
{
...
static result_t actor_pressure_set_state(actor_t **actor, list_t *attributes)
{
	return actor_pressure_init(actor, attributes);
}

static bool actor_pressure_fire(actor_t *actor)
{
...
result_t actor_pressure_register(list_t **actor_types)
{
	...
	type->init = actor_pressure_init;
	type->set_state = actor_pressure_set_state;
	type->free_state = actor_pressure_free;
	type->fire_actor = actor_pressure_fire;
	type->get_managed_attributes = NULL;
	type->will_migrate = NULL;
	type->will_end = NULL;
	type->did_migrate = NULL;

	return list_add_n(actor_types, "sensor.TriggeredPressure", 24, type, sizeof(actor_type_t *));
